...
public class LoadURLContents implements Runnable {
    ...
    public void run() {
        ...
        Log.d(JSONActivity.LOADWEBTAG, threadAndClass + ": run() called, starting load");
        ...
        try {
            ...
            if((actualContentType != null) && (actualContentType.contains(";"))) {
                Log.d(JSONActivity.LOADWEBTAG, threadAndClass + ": Complete HTTP content-type header from server = " + actualContentType);
                int beginparam = actualContentType.indexOf(";", 0);
                actualContentType = actualContentType.substring(0, beginparam);
            }
            Log.d(JSONActivity.LOADWEBTAG, threadAndClass + ": MIME type reported by server = " + actualContentType);
        ...
        Log.d(JSONActivity.LOADWEBTAG, threadAndClass + ": load complete, sending message to UI thread");
